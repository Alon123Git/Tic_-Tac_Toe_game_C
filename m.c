...
bool isPositionTaken(char board[3][3], int pos); // function prototype
...
void whoStart(int *isBegin, bool *isTurn, char board[3][3], int i, int j); // function prototype
...
int main() {
    while (true) {
        if (!isStart) {
            whoStart(&isBegin, &isTurn, board, i, j); // call method to choose who start the game
            isStart = true;
        }
...
void choosePos(char board[3][3], int pos, int O, int X, int isTurn) {
    printf("choose pos ");
    scanf("%d", &pos);
    while (pos < 1 || pos > 9 || isPositionTaken(board, pos)) {
        printf("Invalid position or position taken\nchoose position\n");
        printBoard(board, i, j); // cal method to print the board    
        scanf("%d", &pos);
    }
    if (isTurn) {
...
/**
 * 4 lines
 * @return true if the position is taken, else false
*/
bool isPositionTaken(char board[3][3], int pos) {
    int row = (pos - 1) / 3;
    int col = (pos - 1) % 3;
 ...
 * @return void
*/
void whoStart(int *isBegin, bool *isTurn, char board[3][3], int i, int j) {
